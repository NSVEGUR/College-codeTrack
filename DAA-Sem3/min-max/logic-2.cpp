...
{
public:
  int size, size_of_removed, size_of_recreate, MIN, MAX, Second_MIN, Second_MAX;
  void input_array_size();
  void create_array(int *arr);
  void print_array(int *arr, int *removed_arr);
  void remove_duplicate_array(int *arr, int *removed_arr);
  int gaurd(int *arr);
  void find_min_max(int *arr);
  void recreate_array(int *arr, int *recreated_arr);
  void find_second_min_max(int *arr);
  void program(int *arr, int *removed_arr, int *recreated_arr);
};
...
void min_max::print_array(int *arr, int *removed_arr)
{
  int i;
  ...
  cout << endl
       << "The re-created array(removing duplicates) is: " << endl;
  cout << "[ ";
  for (i = 0; i < this->size_of_removed - 1; ++i)
  {
    cout << *(removed_arr + i) << " ,";
  }

  cout << *(removed_arr + i) << " ]" << endl
       << endl;
}
...
{
  for (int i = 1; i < this->size_of_removed; ++i)
  {
    if (this->MIN > *(arr + i))
      this->MIN = *(arr + i);
    if (this->MAX < *(arr + i))
...
void min_max::recreate_array(int *arr, int *recreated_arr)
{
  int j = 0;
  for (int i = 0; i < this->size_of_removed; ++i)
  {
    if (*(arr + i) == this->MIN || *(arr + i) == this->MAX)
      continue;
    *(recreated_arr + j) = *(arr + i);
    ++j;
  }
  this->size_of_recreate = j;
}

void min_max::find_second_min_max(int *arr)
{
  this->Second_MAX = *(arr);
  this->Second_MIN = *(arr);

  for (int i = 1; i < this->size_of_recreate; ++i)
  {
    if (this->Second_MIN > *(arr + i))
      this->Second_MIN = *(arr + i);
    if (this->Second_MAX < *(arr + i))
      this->Second_MAX = *(arr + i);
  }
}

void min_max::program(int *arr, int *removed_arr, int *recreated_arr)
{
  this->create_array(arr);
  this->remove_duplicate_array(arr, removed_arr);
  this->print_array(arr, removed_arr);
  if (!this->gaurd(removed_arr))
    exit(1);
  this->find_min_max(removed_arr);
  this->recreate_array(removed_arr, recreated_arr);
  this->find_second_min_max(recreated_arr);
...
int main()
{
  ...
  int recreated_arr[min_max_class.size];
  min_max_class.program(arr, removed_arr, recreated_arr);
  return 0;
}